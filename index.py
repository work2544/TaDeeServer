...
import sebackend.quizparser as quizparser
...
def upload_file():
    ...
    try:
        ...
        quizgroup = quizparser(quiztext).Do_parser()
    ...
    return jsonify(f"Uploading file {file.filename} 'author':{author},'category':{category},'createDate':{createdate},'status':{status},'qData':{reader} ")
